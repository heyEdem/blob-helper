...
public class FailureEventRepository {
    ...
    public List<Failure> findSince(Instant since) { return jdbc.query("SELECT * FROM failure_event WHERE occurred_at >= ? ORDER BY occurred_at DESC", (r,n) -> new Failure(UUID.fromString(r.getString("id")), UUID.fromString(r.getString("instance_id")), Instant.parse(r.getString("occurred_at")), r.getString("operation"), r.getString("message")), since.toString()); }
    public int deleteOlderThan(Instant cutoff) { return jdbc.update("DELETE FROM failure_event WHERE occurred_at < ?", cutoff.toString()); }
}
