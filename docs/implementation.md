...
- `blob-helper-jpa/src/main/java/com/edem/blobhelper/jpa/ReferenceCountService.java`: lock-aware service that retains and releases one reference at a time, rejects missing content, and prevents reference-count underflow while leaving transaction ownership with the caller.
