...
- `blob-helper-spring-boot-starter/src/main/java/com/edem/blobhelper/service/DefaultBlobDeduplicationService.java`: default facade that delegates reference mutation and content retrieval to JPA/storage collaborators and orchestrates new uploads by buffering bytes, hashing with `ContentHasher`, generating a deterministic key, writing through `BlobStorage`, and creating metadata through `AssetContentMutationService`.
...
- `blob-helper-spring-boot-starter/src/test/java/com/edem/blobhelper/service/BlobDeduplicationServiceTest.java`: verifies new-content upload orchestration against Hibernate/H2 with a recording storage fake: one physical write, one metadata row with `ref_count = 1`, and a non-duplicate reference.
...
- **Key classes/functions:** `BlobHelperProperties` binds `blob-helper.storage.provider`, `storage.key-prefix`, deduplication hash and upload validation settings, and cleanup deletion/reconciliation settings. `BlobDeduplicationService` exposes storage-neutral `store`, `retain`, `release`, and `get` operations using core models. `DefaultBlobDeduplicationService` buffers upload bytes, hashes them with `ContentHasher`, generates a deterministic key through `ObjectKeyStrategy`, writes once through `BlobStorage`, and persists `AssetContent` through `AssetContentMutationService`, while delegating retain/release and locked metadata retrieval to `blob-helper-jpa`. `deduplication.max-upload-size` uses Spring Boot `DataSize` binding, so values such as `25MB` are accepted.
- **Initialization:** Built as a Maven child of root `blob-helper`; it depends on the provider-neutral core and JPA metadata modules while Spring Boot auto-configuration and duplicate-upload short-circuiting remain later tasks.
- **Non-obvious logic:** Reconciliation is disabled by default, physical deletion on zero references is enabled by default, service callers retain transaction ownership, and the starter has no REST controllers or provider SDK dependencies. Upload streams are buffered once so hashing and storage can consume equivalent byte sequences; duplicate uploads are handled by a later task.
