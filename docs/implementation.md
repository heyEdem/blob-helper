...
- `blob-helper-spring-boot-starter/src/main/java/com/edem/blobhelper/reconcile/ReconciliationService.java`: read-only reconciliation service that compares every stored `AssetContent.ref_count` with application-provided counts and treats omitted content IDs as zero expected references.
...
- `blob-helper-spring-boot-starter/src/test/java/com/edem/blobhelper/reconcile/ReconciliationServiceTest.java`: Hibernate/H2 integration coverage for mismatch reporting, omitted IDs, checked-row totals, and no metadata mutation.
